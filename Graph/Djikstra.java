...
public class Djikstra {
 ...
 static class Node implements Comparable<Node>{
	...
	public static void Djikstra(AdjacencyList graph,int S)
	{
		...
		while(minHeap.isEmpty()==false)
		{
			...
			if(graph.getColour(u.node)==false)
			{
				...
				for(Edge v:AL)
				{
					if(graph.getColour(v.target)==false && n[v.target].distance>(n[u.node].distance+v.weight))
					{
						...
						n[v.target].distance=n[u.node].distance+v.weight;
