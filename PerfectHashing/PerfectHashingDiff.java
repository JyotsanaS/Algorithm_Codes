...
public class PerfectHashingDiff {
	...
	public static void main(String[] args) {
		PerfectHashingDiff ph=new PerfectHashingDiff();
		...
		//Checking if diff <=10 value exist in any of the bucket
		int flagDuplicate=0;
		for(int i=0;i<m;i++)
		{
			for(int j=0;j<hash[i].getn();j++)
			{
				...
				for(int k=1;k<hash[i].getn();k++)
				{
					...
					{
						if(((long)hash[i].getAL().get(k)-(long)hash[i].getAL().get(j))<=-10||((long)hash[i].getAL().get(k)-(long)hash[i].getAL().get(j))<=10)
							{System.out.println(i+""+hash[i].getAL().toString());
							 System.out.println("Elements having difference of 10:"+hash[i].getAL().get(k)+" "+hash[i].getAL().get(j));flagDuplicate=1;}
					}
					if(flagDuplicate==1)
						break;
				}
		...
		if(flagDuplicate==0) //System.out.println("Duplicate Elements exist");
		//else 
			System.out.println("No Elements with difference of 10 exists");
	}
